...
if type==0 | type~=1
    area = zeros(1,N+1);
    % Calculate normalization factor to avoid numerical problems with
    % really small areas - since we're only interested in the area ratios,
    % this normalization factor cancels out
    nf = max(b(2:end-1));
    
    area(2:N)=(xa(3:end-1)-xa(2:end-2)).*exp(b(2:end-1)-nf);
    
    area(1)=(1/m(1)).*exp(m(1)*xa(2)+b(1)-nf)-(1/m(1)).*exp(m(1)*xa(1)+b(1)-nf);
    area(N+1)=(1/m(N+1)).*exp(m(N+1)*xa(N+2)+b(N+1)-nf)-(1/m(N+1)).*exp(m(N+1)*xa(N+1)+b(N+1)-nf);
%%%%%%%%%%%%%%%%%
elseif type==1
%%%%%%%%%%%%%%%%%%
    area = zeros(1,N+1);
    ...
    % really small areas - since we're only interested in the area ratios,
    % this normalization factor cancels out
    nf = max(m(2:end-1).*xa(2:end-2)+b(2:end-1));
    
    fs1=exp(m(2:end-1).*xa(2:end-2)+b(2:end-1)-nf);
    fs2=exp(m(2:end-1).*xa(3:end-1)+b(2:end-1)-nf);  
...
w=area./sum(area);
if(any(isnan(w)))
    fprintf('Uh oh! Something went wrong with our weight calculations.\n');
end
